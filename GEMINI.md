...
- **Обов'язкове використання `AppLink` або `getLocalizedHref`:** Всі внутрішні посилання (крім зовнішніх `http://`, `mailto:`, `tel:` та контенту зі старого сайту) **повинні містити мовний префікс** (`/ua/` або `/ru/`). Для кореневої української сторінки посилання — `/`. 
- **Заборонено використовувати `next/link` з прямими відносними шляхами без мовного каталогу** (наприклад, `href="/our-stores"` чи `href="/actions"`). Для всіх нових компонентів обов'язково використовувати `<AppLink href="...">` з `@/app/components/ui/AppLink/AppLink`.
...
- ❌ Браузерний агент не має права самостійно повністю оформлювати замовлення (фіналізувати/підтверджувати створення замовлення на бекенді чи через UI).
