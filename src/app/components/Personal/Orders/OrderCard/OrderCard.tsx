...
export default function OrderCard({
    ...
    return (
        <div className={s.orderCard}>
            ...
            {/* === ЛІВА КОЛОНКА: номер, статус, зображення, мета+кнопки === */}
            <div className={s.leftCol}>

                {/* Номер замовлення */}
                <span className={s.orderNumber}>
                    {dict.orderPrefix} <strong>№{orderNumber}</strong>
                </span>

                {/* Статус (ліва колонка) + Джерело (мобайл: тут; десктоп: у rightCol) */}
                <div className={s.statusSourceRow}>
                    <div className={s.statusBlock}>
                        {statusIcon}
                        <span className={clsx(s.statusText, s[`status_${statusVariant}`])}>{status}</span>
                    </div>
                    {/* Джерело — тільки мобайл */}
                    <span className={clsx(s.sourceText, s.sourceTextMobile)}>
                        {dict.sourcePrefix}: <strong>{source}</strong>
                    </span>
                </div>
                ...
                <div className={s.priceBlockMobile}>
                    ...
                    <div className={s.priceValues}>
                        <span className={clsx(s.currentPrice, oldSum && s.hasOldPrice)}>
                            {sum.toLocaleString('ru-RU')} <span className={s.currency}>₴</span>
                        </span>
                        {oldSum && (
                            <span className={s.oldPrice}>
                                {oldSum.toLocaleString('ru-RU')} <span className={s.currency}>₴</span>
                            </span>
                        )}
                    </div>
                </div>

                {/* Мета + кнопка "Повторити" в одному рядку */}
                <div className={s.metaRepeatRow}>
                    <div className={s.metaItems}>
                        ...
                        {onReview && (
                            <button type="button" className={clsx(s.metaItem, s.reviewLink)} onClick={onReview}>
                                ...
                                <span>{reviewLabel || dict.reviewLink}</span>
                            </button>
                        )}
                    </div>
                ...
                {/* Кнопка "Деталі" — тільки мобайл */}
                <Button variant="black" className={clsx(s.detailsBtn, s.detailsBtnMobile)} onClick={onDetails}>
                    {dict.detailsBtn}
                </Button>
            </div>

            {/* === ПРАВА КОЛОНКА: джерело, ціна, кнопка "Деталі" (тільки десктоп) === */}
