                ...
                if (editId) {
                    ...
                    if (token) {
                        ...
                        if (list) {
                            ...
                            if (list.createdAt) {
                                try {
                                    const normalized = list.createdAt.replace(' ', 'T');
                                    const date = new Date(normalized);
                                    if (!isNaN(date.getTime())) {
                                        setListDate(date.toLocaleDateString(lang === 'ru' ? 'ru-RU' : 'uk-UA'));
                                    } else {
                                        setListDate(list.createdAt.split(' ')[0] || list.createdAt);
                                    }
                                } catch {
                                    setListDate(list.createdAt);
                                }
                            }
