...
import { getUserDiscountInfoApi, UserDiscountInfo, DiscountPageData } from '@/lib/graphql';
...
const loyaltyDict = {
    ua: {
        ...
        steps: [
            { id: "01", from: "1 грн", to: "8000 грн",  percent: "1%" },
            { id: "02", from: "8001 грн", to: "16000 грн", percent: "2%" },
            { id: "03", from: "16001 грн", to: "30000 грн", percent: "3%" },
            { id: "04", from: "30001 грн", to: null,         percent: "4%" }
        ],
...
interface LoyaltyClientProps {
    ...
    discountPageData?: DiscountPageData | null;
}

export default function LoyaltyClient({ lang, initialDiscountInfo, initialTerms, discountPageData }: LoyaltyClientProps) {
    ...
    const cmsContent = discountPageData?.discountProgramContent;
                    ...
                    <div className={s.dashboard}>
                        <div className={s.leftColumn}>
                            <div className={s.balanceCard}>
                                {cmsContent?.block1Text ? (
                                    <div dangerouslySetInnerHTML={{ __html: cmsContent.block1Text }} />
                                ) : (
                                    <>
                            ...
                            <div className={s.progressCard}>
                                <div className={s.progressTextWrapper}>
                                    {cmsContent?.block2Text ? (
                                        <div dangerouslySetInnerHTML={{ __html: cmsContent.block2Text }} className={s.progressDesc} />
                                    ) : (
                        ...
                        <div className={s.stepsCard}>
                            <div className={s.promoBox}>
                                ...
                                {cmsContent?.block3TopText ? (
                                    <span className={s.cashbackLabel} dangerouslySetInnerHTML={{ __html: cmsContent.block3TopText }} />
                                ) : (
                            ...
                            <div className={s.timelineWrapper}>
                                {cmsContent?.block3StepsText ? (
                                    <div className={s.timelineLabel} dangerouslySetInnerHTML={{ __html: cmsContent.block3StepsText }} />
                                ) : (
                                            ...
                                            <div key={step.id} className={s.stepItem}>
                                                ...
                                                <div className={s.stepInfo}>
                                                    <div className={s.stepRange}>
                                                        Від <strong>{step.from}</strong>
                                                        {step.to && <> до <strong>{step.to}</strong></>}
                                                        {isLast && <> і більше</>} — <span className={s.redText}>{stepPercent}</span> балами <br/> від суми чеку на рахунок клієнта;
                                                    </div>
                                                </div>
                                            </div>
                                        );
