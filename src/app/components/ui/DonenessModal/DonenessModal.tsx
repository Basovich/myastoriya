...
import DonenessSelector from '@/app/pages/Product/DonenessSelector';
import type { ProductCostVariant } from '@/lib/graphql';
import { getProductCostVariantsApi, getDefaultCostVariant } from '@/lib/graphql';
import s from './DonenessModal.module.scss';

interface DonenessModalProps {
    isOpen: boolean;
    onClose: () => void;
    productId: string | number;
    initialVariants?: ProductCostVariant[];
    lang?: 'ua' | 'ru';
    onConfirm: (costVariantId: number) => Promise<void>;
}
...
const translations = {
    ...
    ru: {
        title: 'Выберите степень прожарки',
...
export default function DonenessModal({
    isOpen,
    onClose,
    productId,
    initialVariants,
    ...
    const t = translations[lang === 'ru' ? 'ru' : 'ua'];

    const [variants, setVariants] = useState<ProductCostVariant[]>(initialVariants ?? []);
    ...
    useEffect(() => {
        ...
        if (initialVariants && initialVariants.length > 0) {
            ...
            const def = getDefaultCostVariant(initialVariants);
            setSelectedVariantId(def ? def.id : initialVariants[0].id);
        } else {
            setIsLoading(true);
            getProductCostVariantsApi(productId, lang)
                .then((res) => {
                    setVariants(res);
                    const def = getDefaultCostVariant(res);
                    setSelectedVariantId(def ? def.id : res[0]?.id ?? '');
                })
                .catch((err) => {
                    console.error('[DonenessModal] Failed to load cost variants:', err);
                })
                .finally(() => {
                    setIsLoading(false);
                });
        }
    }, [isOpen, productId, initialVariants, lang]);

    const handleConfirm = async () => {
        if (isSubmitting || !selectedVariantId) return;
        setIsSubmitting(true);
        try {
            await onConfirm(Number(selectedVariantId));
            onClose();
        } catch (err) {
            console.error('[DonenessModal] Error submitting doneness selection:', err);
        } finally {
    ...
    return (
        <Modal
            ...
            closeTimeoutMS={200}
        >
            <button className={s.closeBtn} onClick={onClose} aria-label={t.close}>
            ...
            <div className={s.modal}>
                ...
                <Button
                    ...
                    disabled={isSubmitting || isLoading || !selectedVariantId}
                >
                    {isSubmitting ? t.adding : t.buy}
                </Button>
            </div>
        </Modal>
    );
