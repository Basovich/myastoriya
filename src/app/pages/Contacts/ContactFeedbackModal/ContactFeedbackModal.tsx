'use client';

import React, { useState } from 'react';
import Modal from 'react-modal';
import InputField from '@/app/components/ui/InputField';
import TextareaField from '@/app/components/ui/TextareaField';
import Button from '@/app/components/ui/Button/Button';
import s from './ContactFeedbackModal.module.scss';
import { type Locale } from '@/i18n/config';

interface ContactFeedbackModalProps {
    isOpen: boolean;
    onClose: () => void;
    lang: Locale;
}

const translations = {
    ua: {
        title: "Написати листа",
        nameLabel: "Ім'я*",
        namePlaceholder: "Введіть ваше ім'я",
        emailLabel: "E-mail*",
        emailPlaceholder: "example@mail.com",
        messagePlaceholder: "Введіть текст вашого повідомлення...",
        submitBtn: "Надіслати",
        successText: "Дякуємо! Ваше повідомлення успішно надіслано.",
        errors: {
            nameRequired: "Заповніть це поле",
            emailRequired: "Заповніть це поле",
            emailInvalid: "Некоректний формат E-mail",
            messageRequired: "Заповніть це поле"
        }
    },
    ru: {
        title: "Написать письмо",
        nameLabel: "Имя*",
        namePlaceholder: "Введите ваше имя",
        emailLabel: "E-mail*",
        emailPlaceholder: "example@mail.com",
        messagePlaceholder: "Введите текст вашего сообщения...",
        submitBtn: "Отправить",
        successText: "Спасибо! Ваше сообщение успешно отправлено.",
        errors: {
            nameRequired: "Заполните это поле",
            emailRequired: "Заполните это поле",
            emailInvalid: "Некорректный формат E-mail",
            messageRequired: "Заполните это поле"
        }
    }
};

export default function ContactFeedbackModal({ isOpen, onClose, lang }: ContactFeedbackModalProps) {
    const t = translations[lang] || translations.ua;

    const [name, setName] = useState('');
    const [email, setEmail] = useState('');
    const [message, setMessage] = useState('');
    const [errors, setErrors] = useState<{ name?: string; email?: string; message?: string }>({});
    const [touched, setTouched] = useState(false);
    const [isSubmitted, setIsSubmitted] = useState(false);
    const [isSubmitting, setIsSubmitting] = useState(false);

    const handleClose = () => {
        onClose();
        setTimeout(() => {
            setName('');
            setEmail('');
            setMessage('');
            setErrors({});
            setTouched(false);
            setIsSubmitted(false);
            setIsSubmitting(false);
        }, 200);
    };

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        setTouched(true);

        const newErrors: { name?: string; email?: string; message?: string } = {};

        if (!name.trim()) {
            newErrors.name = t.errors.nameRequired;
        }

        if (!email.trim()) {
            newErrors.email = t.errors.emailRequired;
        } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())) {
            newErrors.email = t.errors.emailInvalid;
        }

        if (!message.trim()) {
            newErrors.message = t.errors.messageRequired;
        }

        if (Object.keys(newErrors).length > 0) {
            setErrors(newErrors);
            return;
        }

        setErrors({});
        setIsSubmitting(true);

        // Simulated submit (since no backend API exists yet)
        setTimeout(() => {
            setIsSubmitting(false);
            setIsSubmitted(true);

            setTimeout(() => {
                handleClose();
            }, 1500);
        }, 400);
    };

    return (
        <Modal
            isOpen={isOpen}
            onRequestClose={handleClose}
            className={s.modalContainer}
            overlayClassName={s.modalOverlay}
            ariaHideApp={false}
            closeTimeoutMS={200}
        >
            <button className={s.closeBtn} onClick={handleClose} aria-label={lang === 'ru' ? 'Закрыть' : 'Закрити'}>
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                    <path d="M11.7625 9.99893L19.6326 2.14129C19.8678 1.90606 20 1.58701 20 1.25434C20 0.921668 19.8678 0.602622 19.6326 0.367388C19.3974 0.132153 19.0783 0 18.7457 0C18.413 0 18.0939 0.132153 17.8587 0.367388L10.0011 8.23752L2.14342 0.367388C1.90819 0.132153 1.58914 2.95361e-07 1.25647 2.97839e-07C0.9238 3.00318e-07 0.604754 0.132153 0.369519 0.367388C0.134285 0.602622 0.00213223 0.921668 0.00213223 1.25434C0.00213223 1.58701 0.134285 1.90606 0.369519 2.14129L8.23966 9.99893L0.369519 17.8566C0.252431 17.9727 0.159496 18.1109 0.0960746 18.2631C0.0326529 18.4153 0 18.5786 0 18.7435C0 18.9084 0.0326529 19.0717 0.0960746 19.224C0.159496 19.3762 0.252431 19.5143 0.369519 19.6305C0.485651 19.7476 0.623817 19.8405 0.776047 19.9039C0.928277 19.9673 1.09156 20 1.25647 20C1.42138 20 1.58467 19.9673 1.7369 19.9039C1.88913 19.8405 2.02729 19.7476 2.14342 19.6305L10.0011 11.7603L17.8587 19.6305C17.9748 19.7476 18.113 19.8405 18.2652 19.9039C18.4175 19.9673 18.5807 20 18.7457 20C18.9106 20 19.0739 19.9673 19.2261 19.9039C19.3783 19.8405 19.5165 19.7476 19.6326 19.6305C19.7497 19.5143 19.8426 19.3762 19.9061 19.224C19.9695 19.0717 20.0021 18.8907 20.0021 18.7435C20.0021 18.5786 19.9695 18.4153 19.9061 18.2631C19.8426 18.1109 19.7497 17.9727 19.6326 17.8566L11.7625 9.99893Z" fill="white" />
                </svg>
            </button>

            <div className={s.modal}>
                {isSubmitted ? (
                    <div className={s.successMessage}>
                        <div className={s.successIcon}>
                            <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M20 6L9 17L4 12" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                            </svg>
                        </div>
                        <span className={s.successText}>{t.successText}</span>
                    </div>
                ) : (
                    <>
                        <h3 className={s.title}>{t.title}</h3>
                        <form className={s.form} onSubmit={handleSubmit} noValidate>
                            <InputField
                                id="feedback-name"
                                name="name"
                                label={t.nameLabel}
                                placeholder={t.namePlaceholder}
                                value={name}
                                onChange={(e) => {
                                    setName(e.target.value);
                                    if (touched) setErrors((prev) => ({ ...prev, name: undefined }));
                                }}
                                error={errors.name}
                                touched={touched}
                            />

                            <InputField
                                id="feedback-email"
                                name="email"
                                type="email"
                                label={t.emailLabel}
                                placeholder={t.emailPlaceholder}
                                value={email}
                                onChange={(e) => {
                                    setEmail(e.target.value);
                                    if (touched) setErrors((prev) => ({ ...prev, email: undefined }));
                                }}
                                error={errors.email}
                                touched={touched}
                            />

                            <TextareaField
                                id="feedback-message"
                                name="message"
                                label={t.messagePlaceholder}
                                value={message}
                                onChange={(e) => {
                                    setMessage(e.target.value);
                                    if (touched) setErrors((prev) => ({ ...prev, message: undefined }));
                                }}
                                error={errors.message}
                                touched={touched}
                                rows={4}
                            />

                            <Button
                                type="submit"
                                variant="red"
                                className={s.submitBtn}
                                disabled={isSubmitting}
                            >
                                {t.submitBtn}
                            </Button>
                        </form>
                    </>
                )}
            </div>
        </Modal>
    );
}
