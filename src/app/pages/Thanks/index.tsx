...
const translations = {
    ua: {
        ...
        breadcrumbsThanks: "Дякуємо за замовлення",
    ...
    ru: {
        ...
        breadcrumbsThanks: "Спасибо за заказ",
                ...
                <div className={s.bannerContainer}>
                    ...
                    <div className={s.bannerContent}>
                        <p className={s.bannerText}>
                            {lang === "ua" ? (
                                <>
                                    ЦЕ ВАШЕ ПЕРШЕ ЗАМОВЛЕННЯ, ТОМУ МИ ВАМ <span className={s.highlight}>ДАРУЄМО</span> <span className={s.highlight}>ПРОМОКОД ЗІ ЗНИЖКОЮ 10%</span> НА ВАШЕ НАСТУПНЕ ЗАМОВЛЕННЯ!
                                </>
                            ) : (
                                <>
                                    ЭТО ВАШ ПЕРВЫЙ ЗАКАЗ, ПОЭТОМУ МЫ ВАМ <span className={s.highlight}>ДАРИМ</span> <span className={s.highlight}>ПРОМОКОД СО СКИДКОЙ 10%</span> НА ВАШ СЛЕДУЮЩИЙ ЗАКАЗ!
                                </>
                            )}
                        </p>
                        <div className={s.promoBlock} onClick={handleCopyPromo} role="button" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && handleCopyPromo()}>
                            ...
                            <span className={s.arrows} aria-hidden="true">&gt;&gt;</span>
                        </div>
                    </div>
                </div>
